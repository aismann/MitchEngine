...
namespace Moonlight
{
	void LegacyRenderer::UpdateMatrix(unsigned int Id, DirectX::SimpleMath::Matrix NewTransform)
	{
	...
	void LegacyRenderer::UpdateMeshMatrix(unsigned int Id, DirectX::SimpleMath::Matrix NewTransform)
	{
		if (Id >= Meshes.size())
		{
			return;
	...
	void LegacyRenderer::UpdateCamera(unsigned int Id, CameraData& NewCommand)
	{
		if (Id >= Cameras.size())
		{
			return;
	...
	Moonlight::CameraData& LegacyRenderer::GetCamera(unsigned int Id)
	{
		if (Id >= Cameras.size())
		{
			return Cameras[0];
...
*/
	...
	void LegacyRenderer::ResizeBuffers()
	{
		delete SceneViewRTT;
	...
	void LegacyRenderer::SetWindow()
	{

	}

	void LegacyRenderer::RegisterDeviceNotify(IDeviceNotify* deviceNotify)
	{

	}

	LegacyDX11Device& LegacyRenderer::GetDevice()
	{
		return *m_device;
	}

	void LegacyRenderer::Update(float dt)
	{
	}

	void LegacyRenderer::Render(std::function<void()> func, std::function<void()> uiRender, const CameraData& editorCamera)
	{
	...
	void LegacyRenderer::DrawScene(ID3D11DeviceContext3* context, ModelViewProjectionConstantBuffer& constantBufferSceneData, const CameraData& camera, FrameBuffer* ViewRTT)
	{
...
// 				}
	...
	void LegacyRenderer::DrawDepthOnlyScene(ID3D11DeviceContext3* context, DepthPassBuffer& constantBufferSceneData, FrameBuffer* ViewRTT)
	{
	...
	void LegacyRenderer::DrawPickingTexture(ID3D11DeviceContext3* context, PickingConstantBuffer& constantBufferSceneData, const CameraData& camera, FrameBuffer* ViewRTT)
	{
		Vector2 outputSize = camera.OutputSize;
		if (outputSize.IsZero())
		{
	...
	void LegacyRenderer::ReleaseDeviceDependentResources()
	{
		m_constantBuffer.Reset();
		m_perFrameBuffer.Reset();
	}

	void LegacyRenderer::CreateDeviceDependentResources()
	{
		CD3D11_BUFFER_DESC constantBufferDesc(sizeof(ModelViewProjectionConstantBuffer), D3D11_BIND_CONSTANT_BUFFER);
		DX::ThrowIfFailed(
			static_cast<LegacyDX11Device*>(m_device)->GetD3DDevice()->CreateBuffer(
				&constantBufferDesc,
				nullptr,
				&m_constantBuffer
			)
		...
		DX::ThrowIfFailed(
			static_cast<LegacyDX11Device*>(m_device)->GetD3DDevice()->CreateBuffer(&perFrameBufferDesc, nullptr, &m_perFrameBuffer)
		);
		//hr = d3d11Device->CreateBuffer(&cbbd, NULL, &cbPerFrameBuffer);
	}

	unsigned int LegacyRenderer::PushDebugCollider(const DebugColliderCommand& NewModel)
	{
	...
	bool LegacyRenderer::PopDebugCollider(unsigned int id)
	{
		if (id >= DebugColliders.size())
		{
			return false;
	...
	void LegacyRenderer::ClearDebugColliders()
	{
	...
	unsigned int LegacyRenderer::PushLight(const LightCommand& NewLight)
	{
	...
	bool LegacyRenderer::PopLight(unsigned int id)
	{
		if (id >= Lights.size())
		{
			return false;
	...
	void LegacyRenderer::ClearLights()
	{
		Lights.clear();
	...
	void LegacyRenderer::WindowResized(const Vector2& NewSize)
	{
		if (!m_device)
		{
			return;
	...
	void LegacyRenderer::PickScene(const Vector2& Pos)
	{
...
//			SaveTextureToBmp(StringUtils::ToWString(Path("Assets/EXPORT.bmp").FullPath).c_str(), SceneViewRTT->PickingTexture.Get(), Pos);
	...
	unsigned int LegacyRenderer::PushMesh(Moonlight::MeshCommand command)
	{
		switch (command.Type)
		{
		case MeshType::Plane:
	...
	void LegacyRenderer::PopMesh(unsigned int Id)
	{
		if (Id > Meshes.size())
		{
			return;
	...
	void LegacyRenderer::ClearMeshes()
	{
		Meshes.clear();
	...
	unsigned int LegacyRenderer::PushCamera(Moonlight::CameraData& command)
	{
	...
	void LegacyRenderer::PopCamera(unsigned int Id)
	{
		if (Id > Cameras.size())
		{
			return;
	...
	void LegacyRenderer::ClearUIText()
	{
		Cameras.clear();
	...
	void LegacyRenderer::SaveTextureToBmp(PCWSTR FileName, ID3D11Texture2D* Texture, const CameraData& camera, const Vector2& Pos)
	{
		HRESULT hr;
